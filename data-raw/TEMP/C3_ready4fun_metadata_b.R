...
make_new_ready4fun_metadata_b <- function(x) {
  stopifnot(is.list(x))
  class(x) <- append(
    c("ready4fun_metadata_b", setdiff(make_pt_ready4fun_metadata_b() %>% class(), class(x))),
    class(x)
  )
  x
}
...
make_pt_ready4fun_metadata_b <- function(
...
validate_ready4fun_metadata_b <- function(x) {
  if (sum(stringr::str_detect(
    names(x)[names(x) %in% names(make_pt_ready4fun_metadata_b())],
    names(make_pt_ready4fun_metadata_b())
  )) != length(names(make_pt_ready4fun_metadata_b()))) {
    stop(
      paste0(
