...
make_new_ready4fun_manifest <- function(x) {
  stopifnot(is.list(x))
  class(x) <- append(
    c("ready4fun_manifest", setdiff(make_pt_ready4fun_manifest() %>% class(), class(x))),
    class(x)
  )
  x
}
...
make_pt_ready4fun_manifest <- function(
    initial_ls = ready4fun_metadata_a(),
    subsequent_ls = ready4fun_metadata_b()) {
  args_ls <- list(
    initial_ls = initial_ls,
    subsequent_ls = subsequent_ls
...
validate_ready4fun_manifest <- function(x) {
  if (sum(stringr::str_detect(
    names(x)[names(x) %in% names(make_pt_ready4fun_manifest())],
    names(make_pt_ready4fun_manifest())
  )) != length(names(make_pt_ready4fun_manifest()))) {
    stop(
      paste0(
