...
make_new_ready4fun_executor <- function(x) {
  stopifnot(is.list(x))
  class(x) <- append(
    c("ready4fun_executor", setdiff(make_pt_ready4fun_executor() %>% class(), class(x))),
    class(x)
  )
  x
}
...
make_pt_ready4fun_executor <- function(
    args_ls = list(),
    fn = identity) {
  args_ls <- list(
    args_ls = args_ls,
...
validate_ready4fun_executor <- function(x) {
  if (sum(stringr::str_detect(
    names(x)[names(x) %in% names(make_pt_ready4fun_executor())],
    names(make_pt_ready4fun_executor())
  )) != length(names(make_pt_ready4fun_executor()))) {
    stop(
      paste0(
