...
make_new_ready4fun_metadata_a <- function(x) {
  stopifnot(is.list(x))
  class(x) <- append(
    c("ready4fun_metadata_a", setdiff(make_pt_ready4fun_metadata_a() %>% class(), class(x))),
    class(x)
  )
  x
}
...
make_pt_ready4fun_metadata_a <- function(
    pkg_desc_ls = ready4fun_description(),
...
validate_ready4fun_metadata_a <- function(x) {
  if (sum(stringr::str_detect(
    names(x)[names(x) %in% names(make_pt_ready4fun_metadata_a())],
    names(make_pt_ready4fun_metadata_a())
  )) != length(names(make_pt_ready4fun_metadata_a()))) {
    stop(
      paste0(
        "LIST must include elements named: ",
  ...
  if (!identical(
    ...
    stop(
      paste0(
        "LIST elements should be of the following classes: ",
        "",
        {
          ...
          vars_chr <- class_lup %>%
            dplyr::pull(1) %>%
            unique()
          classes_chr <- vars_chr %>% purrr::map_chr(~ dplyr::filter(class_lup, variable == .x) %>%
            dplyr::pull(2) %>%
            paste0(collapse = ", "))
          purrr::map2_chr(
            vars_chr,
