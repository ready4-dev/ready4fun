...
#' @description make_abbr_lup_tb() is a Make function that creates a new R object. Specifically, this function implements an algorithm to make an abbreviation lookup table. The function is called for its side effects and does not return a value.
...
#' @keywords internal
make_arg_type_abbr_chr_vec <- function (fn_args_chr_vec, object_type_lup = NULL) 
{
    if (is.null(object_type_lup)) 
        data("object_type_lup", package = "ready4fun", envir = environment())
...
#' @keywords internal
make_fn_desc_chr_vec <- function (fns_chr_vec, title_chr_vec, output_chr_vec) 
{
    fn_desc_chr_vec <- purrr::pmap_chr(list(fns_chr_vec, title_chr_vec, 
        ...
        paste0(make_fn_desc_spine_chr_vec(fn_name_chr = ..1, 
            fn_title_chr = ..2), " The function ", ifelse(..3 == 
            "NULL", paste0("is called for its side effects and does not return a value.", 
            ifelse(endsWith(..1, "_R"), " WARNING: This function writes R scripts to your local environment. Make sure to only use if you want this behaviour", 
                "")), paste0("returns ", ..3 %>% tolower() %>% 
            add_indef_artl_to_item_chr_vec(), ".")))
    })
...
#' @keywords internal
make_fn_desc_spine_chr_vec <- function (fn_name_chr, fn_title_chr, fn_type_lup_tb = NULL) 
{
    ...
    fn_desc_spine_chr_vec <- paste0(fn_name_chr, "() is ", add_indef_artl_to_item_chr_vec(fn_type_chr_vec[1], 
        ignore_phrs_not_in_lup = F), " function that ", update_first_word_case_chr(text_elements_chr_vec[1]), 
        " Specifically, this function implements an algorithm to ", 
        fn_name_chr %>% remove_obj_type_from_nm_chr_vec() %>% 
            add_indefartls_to_phrases_chr_vec(), ".", ifelse(ifelse(is.null(fn_args_chr_vec) | 
...
make_fn_dmt_tbl_tb <- function (fns_path_chr_vec, fns_dir_chr, pkg_nm_chr, custom_dmt_ls = list(title_ls = NULL, 
    ...
    example_ls = NULL, args_ls_ls = NULL), append_lgl = T) 
{
...
#' @keywords internal
make_fn_dmt_tbl_tpl_tb <- function (fns_path_chr_vec, fns_dir_chr) 
{
...
make_new_fn_dmt_chr_ls <- function (fn_type_chr, fn_name_chr, fn_desc_chr = NA_character_, 
    ...
    args_ls = NULL, fn = NULL, object_type_lup = NULL) 
{
    if (is.null(object_type_lup)) 
        data("object_type_lup", package = "ready4fun", envir = environment())
    s3_class_main <- NULL
    if (!is.null(fn)) {
        ...
        fn_out_type_chr <- ifelse(is.na(fn_out_type_chr), get_return_obj_nm_chr(fn) %>% 
            make_arg_desc_chr_vec(object_type_lup = object_type_lup), 
            fn_out_type_chr)
    }
    else {
