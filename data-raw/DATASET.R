...
badges_lup <- tibble::tibble(badge_names_chr = "ready4",
  ...
  dplyr::mutate(badges_chr = purrr::map2_chr(label_names_chr, colours_chr,
                                             ~badgr::get_badge(
                                               ...
                                               browser_preview = F,
                                               to_clipboard = F)))
pkg_desc_ls <- fns_env_ls$fns_env$make_pkg_desc_ls(pkg_title_1L_chr = "Author and Document Functions that Extend the Ready4 Framework" %>% tools::toTitleCase(),
...
constructor_r3 <- dplyr::bind_rows(
  ...
  ##
  ready4class::make_pt_ready4class_constructor(class_desc_chr = "Object types lookup table.",
                                               make_s3_lgl = TRUE,
  ...
  ##
  ready4class::make_pt_ready4class_constructor(class_desc_chr = "Function types lookup table.",
                                               make_s3_lgl = TRUE,
                                               name_stub_chr = "functions",
                                               pt_ls = list(list("tibble")),
                                               pt_chkr_pfx_ls = list(list("is_")),
                                               pt_ns_ls = list(list("tibble")),
                                               vals_ls = list(list(fn_type_nm_chr = "character(0)",
