    ...
    abbreviations_lup, is_generic_1L_lgl = NULL) 
{
    ...
    pfx_matches_chr <- fn_types_lup$fn_type_nm_chr[purrr::map_lgl(types_chr, 
        ~startsWith(fn_title_1L_chr, .x))]
    ...
    test_for_write_R_warning_fn = NULL) 
{
    ...
    substr(fn_types_chr, 1, 1) <- tolower(substr(fn_types_chr, 
        1, 1))
...
#' @importFrom stringi stri_replace_last_fixed stri_replace_last_regex
#' @keywords internal
    ...
    server_1L_chr = deprecated()) 
{
    ...
    title_chr <- remove_obj_type_from_nm(fns_chr, object_type_lup = object_type_lup, 
        ...
        " ") %>% purrr::map_chr(~replace_abbr(.x, abbreviations_lup = abbreviations_lup) %>% 
        stringi::stri_replace_last_fixed(" R", "")) %>% stringi::stri_replace_last_regex("\\.", 
        "")
    return(title_chr)
}
...
#' @param pkg_dmt_dv_dss_chr Package documentation dataverse datasets (a character vector), Default: deprecated()
...
#' @param cls_fn_ls Class (a list of functions), Default: list()
#' @param custom_dmt_ls Custom documentation (a list), Default: make_custom_dmt_ls()
...
#' @param user_manual_fns_chr User manual functions (a character vector), Default: 'NA'
#' @param zenodo_badge_1L_chr Zenodo badge (a character vector of length one), Default: character(0)
    ...
    zenodo_badge_1L_chr = character(0)) 
{
    ...
    if (dir.exists(paste0(path_to_pkg_rt_1L_chr, "/tests"))) {
        ...
        testcov_badge_1L_chr <- paste0("[![", "test-coverage", 
            ...
            ")")
        codecov_url_1L_chr <- paste0("https://app.codecov.io/gh/", 
            gh_repo_1L_chr)
    ...
    if (fn_type_1L_chr %in% c("fn", "gen_std_s3_mthd", "meth_std_s3_mthd", 
        ...
        if (fn_type_1L_chr == "meth_std_s3_mthd") {
            submodule_1L_chr <- stringr::str_sub(fn_name_1L_chr, 
                start = (1 + stringi::stri_locate_last_fixed(fn_name_1L_chr, 
                  ".")[1, 1]))
            x_param_desc_1L_chr <- paste0("An instance of `", 
                submodule_1L_chr, "`, a ", ready4::get_from_lup_obj(abbreviations_lup, 
                  match_var_nm_1L_chr = "short_name_chr", match_value_xx = submodule_1L_chr, 
                  target_var_nm_1L_chr = "long_name_chr", evaluate_1L_lgl = F))
        }
...
make_obj_lup <- function (obj_lup_spine = make_obj_lup_spine()) 
{
    ...
    obj_tb <- dplyr::bind_rows(obj_tb %>% dplyr::mutate(long_name_chr = purrr::map2_chr(long_name_chr, 
        ...
        obj_tb %>% dplyr::filter(r3_can_extend_lgl) %>% dplyr::mutate(short_name_chr = paste0(short_name_chr, 
            ...
            long_name_chr = paste0("ready4 submodule extension of ", 
    ...
    obj_tb <- obj_tb %>% dplyr::mutate(plural_lgl = F) %>% dplyr::filter(!long_name_chr %>% 
        startsWith("ready4 S4 collection of"))
    return(obj_tb)
}
    ...
    piggyback_to_1L_chr = "ready4-dev/ready4"), new_entries_tb = NULL) 
{
    if (is.null(seed_obj_type_lup)) {
        seed_obj_type_lup <- tibble::tibble(short_name_chr = c("df", 
            ...
            "environment", "function", "list", "plot", "ready4 submodule", 
            "ready4 module", "S3", "S4", "simple features object", 
