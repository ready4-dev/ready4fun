...
make_new_ready4fun_badges <- function(x) {
  ...
  class(x) <- append(
    c("ready4fun_badges", setdiff(make_pt_ready4fun_badges() %>% class(), class(x))),
    class(x)
  )
  x
}
...
validate_ready4fun_badges <- function(x) {
  if (sum(stringr::str_detect(
    names(x)[names(x) %in% names(make_pt_ready4fun_badges())],
    names(make_pt_ready4fun_badges())
  )) != length(names(make_pt_ready4fun_badges()))) {
    stop(
      paste0(
        "TIBBLE must include columns named: ",
  ...
  if (!identical(
    ...
    stop(
      paste0(
        "TIBBLE columns should be of the following classes: ",
        "",
        {
