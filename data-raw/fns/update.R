...
update_fn_dmt <- function(fn_tags_spine_ls,
  ...
  fn_dmt_1L_chr <- fn_dmt_1L_chr %>%
    stringr::str_replace("FUNCTION_TITLE",
                         ifelse(fn_type_1L_chr == "s3_valid_instance",
                                stringr::str_replace(fn_name_1L_chr,
                                                     pattern = "ready4 S3 class ",
                                                     replacement = ""),
                                fn_name_1L_chr)) %>%
    ...
    fn_dmt_1L_chr <- purrr::reduce(1:length(new_tag_chr_ls$arg_desc_chr),
                                   ...
                                   ~{
                                     stringr::str_replace(.x,
                                                          ...
                                                          paste0("@param ",names(new_tag_chr_ls$arg_desc_chr)[.y]," ",
                                                                 ifelse(new_tag_chr_ls$arg_desc_chr[.y]=="NO MATCH",
                                                                        ifelse(! names(new_tag_chr_ls$arg_desc_chr[.y]) %in% c(letters,
                                                                                                                               LETTERS),#!="x" ## OBJECT LETTER CHANGE
                                                                               "PARAM_DESCRIPTION",
                                                                               "An object"),
                                                                        #"PARAM_DESCRIPTION",
  ...
  if(!is.na(import_chr)){
    ...
    if(import_idx_1L_int != 0){
      ...
      import_txt_1L_chr <- paste0(import_txt_1L_chr,
                                  paste0(" ",
      ...
      import_txt_1L_chr <- paste0("#' @import ",
  ...
  if(gnrc_part_1L_chr %in% names(import_from_chr) & fn_type_1L_chr %in% c("meth_std_s3_mthd",
    ...
    if(!identical(import_idx_1L_int, integer(0))){
      ...
      import_txt_1L_chr <- paste0("#' @importFrom ",
                                  ns_1L_chr,
                                  " ",
