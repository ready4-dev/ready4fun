...
renew.ready4fun_manifest <- function(
    x, type_1L_chr, are_words_chr = character(0), custom_plural_ls = NULL,
  ...
  if (type_1L_chr == "fns_dmt") {
    x <- add_fns_dmt_tb(
      pkg_setup_ls = x, fns_env_ls = NULL,
      inc_methods_1L_lgl = T
    )
  }
  if (!identical(are_words_chr, character(0)) | !identical(
    tf_to_singular_chr,
    character(0)
  ) | !identical(not_obj_type_chr, character(0))) {
