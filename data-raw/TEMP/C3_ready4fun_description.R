...
make_new_ready4fun_description <- function(x) {
  stopifnot(is.list(x))
  class(x) <- append(
    c("ready4fun_description", setdiff(make_pt_ready4fun_description() %>% class(), class(x))),
    class(x)
  )
  x
}
...
make_pt_ready4fun_description <- function(
    Package = character(0),
    Title = character(0),
...
validate_ready4fun_description <- function(x) {
  if (sum(stringr::str_detect(
    names(x)[names(x) %in% names(make_pt_ready4fun_description())],
    names(make_pt_ready4fun_description())
  )) != length(names(make_pt_ready4fun_description()))) {
    stop(
      paste0(
