...
make_new_ready4fun_dataset <- function(x) {
  stopifnot(is.list(x))
  class(x) <- append(
    c("ready4fun_dataset", setdiff(make_pt_ready4fun_dataset() %>% class(), class(x))),
    class(x)
  )
  x
}
...
make_pt_ready4fun_dataset <- function(
    db_df = data.frame(),
...
validate_ready4fun_dataset <- function(x) {
  if (sum(stringr::str_detect(
    names(x)[names(x) %in% names(make_pt_ready4fun_dataset())],
    names(make_pt_ready4fun_dataset())
  )) != length(names(make_pt_ready4fun_dataset()))) {
    stop(
      paste0(
