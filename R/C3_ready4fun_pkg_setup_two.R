...
#' Make new ready4 S4 class for package metadata required for second package set-up step.
...
#' Make prototype ready4 S4 class for package metadata required for second package set-up step.
...
user_manual_fns_chr = user_manual_fns_chr) %>% ready4class::update_pt_fn_args_ls()
rlang::exec(list,!!!args_ls)
}
#' Validate ready4 S4 class for package metadata required for second package set-up step.
...
purrr::map2_chr(make_pt_ready4fun_pkg_setup_two() %>% 
lapply(class) %>% tibble::as_tibble() %>% 
 tidyr::gather(variable,class) %>% 
dplyr::pull(1),
 make_pt_ready4fun_pkg_setup_two() %>% 
lapply(class) %>% tibble::as_tibble() %>% 
 tidyr::gather(variable,class) %>% 
dplyr::pull(2),
 ~ paste0(.x,": ",.y)) %>% 
...
x}
#' Is ready4 S4 class for package metadata required for second package set-up step.
