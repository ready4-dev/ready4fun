...
renew.ready4fun_manifest <- function(x,
  ...
  if (type_1L_chr == "fns_dmt") {
    x <- add_fns_dmt_tb(append_1L_lgl = append_1L_lgl, pkg_setup_ls = x, fns_env_ls = NULL, inc_methods_1L_lgl = T)
  }
  ...
  if (type_1L_chr == "abbreviations") {
    ...
    x <- write_new_abbrs(x, custom_plural_ls = custom_plural_ls, key_1L_chr = key_1L_chr, long_name_chr = long_name_chr, no_plural_chr = no_plural_chr, pfx_rgx = pfx_rgx)
  }
  return(x)
}
