...
make_new_ready4fun_abbreviations <- function(x) {
  ...
  class(x) <- append(
    c("ready4fun_abbreviations", setdiff(make_pt_ready4fun_abbreviations() %>% class(), class(x))),
    class(x)
  )
  x
}
...
make_pt_ready4fun_abbreviations <- function(
...
validate_ready4fun_abbreviations <- function(x) {
  if (sum(stringr::str_detect(
    names(x)[names(x) %in% names(make_pt_ready4fun_abbreviations())],
    names(make_pt_ready4fun_abbreviations())
  )) != length(names(make_pt_ready4fun_abbreviations()))) {
    stop(
      paste0(
